...
void runDFS(int ** edges, int n, int start, bool * visited) {
	visited[start] = true;

	for (int i = 0; i < n; i++) {
		if (i != start and !visited[i] and edges[start][i] == 1) {
			runDFS(edges, n, i, visited);
		}
	}
}
bool isConnnected(int ** edges, int n) {
	bool * visited = new bool[n];

	for (int i = 0; i < n; i++) {
		visited[i] = false;
	}
	runDFS(edges, n, 0 , visited);
